...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2):
    ...
    if partition == 'pat':
        ...
        # Minimum size of samples for each client.
        min_size = 0
        K = num_classes
        # Total number of samples.
        N = len(dataset_label)

        # Ensure each client has at least the minimum number of samples
        while min_size < least_samples:
            ...
            for k in range(K):
                ...
                # Adjust proportions to ensure each client gets samples
                proportions = np.array([p*(len(idx_j)<N/num_clients) for p,idx_j in zip(proportions,idx_batch)])
                # Normalize the proportions
                proportions = proportions/proportions.sum()
                # Calculate cumulative proportions and split indices accordingly
                proportions = (np.cumsum(proportions)*len(idx_k)).astype(int)[:-1]
                # Split indices and assign to clients
                idx_batch = [idx_j + idx.tolist() for idx_j,idx in zip(idx_batch,np.split(idx_k,proportions))]
                # Update the minimum size of samples for each client
                min_size = min([len(idx_j) for idx_j in idx_batch])
        # Map the indices to each client
